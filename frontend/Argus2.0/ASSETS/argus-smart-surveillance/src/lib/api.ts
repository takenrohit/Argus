...
function toUiStatus(sourceStatus: string | undefined, severity: IncidentSeverity): Incident['status'] {
  switch (sourceStatus) {
    ...
    case 'acknowledged':
      return 'dispatching';
    default:
      return severity === 'CRITICAL' ? 'reviewing' : 'active';
  }
}
...
export function normalizeIncident(payload: BackendIncidentPayload): Incident {
  ...
  return {
    ...
    evidenceUrl: payload.frame_b64 ? `data:image/jpeg;base64,${payload.frame_b64}` : undefined,
...
export async function updateIncidentStatus(incidentId: string, status: 'acknowledged' | 'resolved' | 'false_positive') {
...
export function buildAlertsWebSocketUrl() {
  if (WS_BASE_URL) {
